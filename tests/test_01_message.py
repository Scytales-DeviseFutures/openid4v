...
def test_credential_request():
    ...
    sign_alg = "ES256"
    _kid = 0
    _possible_keys = KEYJAR.get_signing_key(
        key_type=alg2keytype(sign_alg), issuer_id=JWT_ISSUER
    )
    if len(_possible_keys) >= 1:
        _kid = _possible_keys[0].kid

    _headers = {"typ": "openid4vci-proof+jwt", "jwk": _possible_keys[0].serialize()}
...
def test_authorization_detail():
    ...
    ad = AuthorizationDetail(
        type="openid_credential", format="jwt_vc_json", credential_definition=cd
    )
...
def check_0(ads):
    ...
    assert ad["credential_definition"]["type"] == [
...
def check_1(ads):
    ...
    assert set(ad["credential_definition"]["credentialSubject"].keys()) == {
        "degree",
        "family_name",
        "given_name",
    }
...
def check_2(ads):
    ...
    ad = ads[0]
    assert set(ad.keys()) == {"type", "format", "credential_definition"}
    assert set(ad["credential_definition"].keys()) == {
        "type",
        "credentialSubject",
        "@context",
    }
...
def test_issuer_metadata():
    ...
    assert (
        len(
            [
                c
                for c in metadata["credentials_supported"]
                if c["format"] == "jwt_vc_json"
            ]
        )
        == 1
    )
...
def test_credential_issuer_metadata():
    ...
    # One I can deal with
    assert (
        len([c for c in metadata["credentials_supported"] if c["format"] == "jwt_vc"])
        == 2
    )
